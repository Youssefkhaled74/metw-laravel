...
<head>
    ...
    <title>@yield('title', __('shipment-dashboard.dashboard')) - {{ config('app.name') }}</title>
    ...
    <style>
        :root {
            ...
            --sidebar-bg-start: #28a745;
            --sidebar-bg-end: #20c997;
            --topbar-bg: #ffffff;
            --topbar-border: #d6efe1;
            --topbar-shadow: 0 10px 28px rgba(17, 24, 39, 0.08);
            --topbar-text: #111827;
            --topbar-accent: #16a34a;
        }
        ...
        .topbar-shell {
            ...
            padding: 1rem 1.1rem;
            background:
                radial-gradient(120px 60px at 100% 0%, rgba(34, 197, 94, 0.10), transparent 70%),
                radial-gradient(120px 60px at 0% 0%, rgba(32, 201, 151, 0.10), transparent 70%);
        }
        ...
        .topbar-control:active {
            border-color: #8ed3a8;
            box-shadow: 0 0 0 0.2rem rgba(40, 167, 69, 0.16);
        }
...
<body>
    <nav class="sidebar" id="shipmentSidebar">
        <div class="sidebar-inner">
            <div class="sidebar-brand">
                <div class="d-flex align-items-center gap-2">
                    <i class="fas fa-truck"></i>
                    <span class="brand-text">@lang('shipment-dashboard.panel_title')</span>
                </div>
            ...
            <div class="sidebar-menu">
                <div class="text-white-50 small mb-3">
                    {{ Auth::guard('shipment')->user()->name ?? '' }}
                </div>
                <ul class="nav flex-column">
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.dashboard') ? 'active' : '' }}"
                           href="{{ route('shipment.dashboard') }}">
                            <i class="fas fa-tachometer-alt"></i>
                            <span class="link-text">@lang('shipment-dashboard.dashboard')</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.orders*') ? 'active' : '' }}"
                           href="{{ route('shipment.orders') }}">
                            <i class="fas fa-shipping-fast"></i>
                            <span class="link-text">@lang('shipment-dashboard.orders')</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.ecommerce.*') ? 'active' : '' }}"
                           href="{{ route('shipment.ecommerce.orders') }}">
                            <i class="fas fa-store"></i>
                            <span class="link-text">@lang('shipment-dashboard.ecommerce_orders')</span>
                        </a>
                    </li>
                    {{-- </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.locations*') ? 'active' : '' }}"
                           href="{{ route('shipment.locations.index') }}">
                            <i class="fas fa-map-marker-alt"></i>
                            <span class="link-text">@lang('shipment-dashboard.locations')</span>
                        </a>
                    </li> --}}
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.shipment-locations*') ? 'active' : '' }}"
                           href="{{ route('shipment.shipment-locations.index') }}">
                            <i class="fas fa-map-marker-alt"></i>
                            <span class="link-text">@lang('shipment-dashboard.shipment_locations')</span>
                        </a>
                    </li>
                    {{-- <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.packages*') ? 'active' : '' }}"
                           href="{{ route('shipment.packages') }}">
                            <i class="fas fa-box"></i>
                            <span class="link-text">@lang('shipment-dashboard.packages')</span>
                        </a>
                    </li> --}}
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.return-requests*') ? 'active' : '' }}"
                           href="{{ route('shipment.return-requests') }}">
                            <i class="fas fa-exchange-alt"></i>
                            <span class="link-text">@lang('shipment-dashboard.return_requests')</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.pricing*') ? 'active' : '' }}"
                           href="{{ route('shipment.pricing') }}">
                            <i class="fas fa-dollar-sign"></i>
                            <span class="link-text">@lang('shipment-dashboard.pricing')</span>
                        </a>
                    </li>
                    {{-- <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.reports') ? 'active' : '' }}"
                           href="{{ route('shipment.reports') }}">
                            <i class="fas fa-chart-bar"></i>
                            <span class="link-text">@lang('shipment-dashboard.reports')</span>
                        </a>
                    </li> --}}
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('shipment.profile*') ? 'active' : '' }}"
                           href="{{ route('shipment.profile') }}">
                            <i class="fas fa-user"></i>
                            <span class="link-text">@lang('shipment-dashboard.profile')</span>
                        </a>
                    </li>
                </ul>
            </div>
        ...
        <div class="page-header-wrapper">
            <div class="topbar-shell d-flex justify-content-between align-items-center pt-3 pb-2 mb-3 border-bottom">
                <div class="page-title-wrapper">
                    <h1 class="h2 mb-0">@yield('page-title', __('shipment-dashboard.page_title_dashboard'))</h1>
                </div>
                ...
                <div class="dropdown me-3">
                    <button class="btn btn-light topbar-control position-relative" id="notificationDropdown"
                        ...
                        @if(auth('shipment')->user()->unreadNotifications->count() > 0)
                            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                                {{ auth('shipment')->user()->unreadNotifications->count() }}
                            </span>
                        @endif
                    </button>
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow"
                        ...
                        <li class="dropdown-header">@lang('shipment-dashboard.notifications')</li>

                        @forelse(auth('shipment')->user()->unreadNotifications as $notification)
                            <li>
                        ...
                        @empty
                            <li><p class="text-center text-muted py-2">@lang('shipment-dashboard.no_notifications')</p></li>
                        @endforelse

                        <li><hr class="dropdown-divider"></li>
                        <li>
                            <a class="dropdown-item text-center text-primary" href="{{ route('shipment.notifications.index') }}">
                                @lang('shipment-dashboard.view_all')
                            </a>
                        </li>
                    </ul>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const sidebar = document.getElementById('shipmentSidebar');
        const storageKey = 'shipmentSidebarCollapsed';
        const sidebarCollapseTriggers = sidebar
            ? Array.from(sidebar.querySelectorAll('a.nav-link[data-bs-toggle="collapse"][href^="#"]'))
            : [];
        ...
        const toggleDesktopState = () => {
            ...
            if (body.classList.contains('sidebar-collapsed')) {
                sidebarCollapseTriggers.forEach(trigger => {
        ...
        const handleResize = () => {
            if (isDesktop()) {
                ...
                sidebarCollapseTriggers.forEach(trigger => {
                    ...
                    target.addEventListener('shown.bs.collapse', syncExpanded);
                    target.addEventListener('hidden.bs.collapse', syncExpanded);
                    syncExpanded();
