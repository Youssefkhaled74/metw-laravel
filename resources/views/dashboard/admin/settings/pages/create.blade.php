...
@section('title', __('admin-dashboard.create_page'))
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('admin-dashboard.create_page') }}</h1>
        ...
        <div class="card-body p-4 p-lg-4">
            <form action="{{ route('admin.settings.pages.store') }}" method="POST">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                                ...
                                @foreach($pageTypes as $type)
                                    <option value="{{ $type->value }}" {{ old('type', request('type')) == $type->value ? 'selected' : '' }}>
                                        {{ $type->label() }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="page-form-section">
                    ...
                    <div class="form-group">
                        ...
                        <select id="validity_mode" name="validity_mode" class="form-control @error('validity_mode') is-invalid @enderror">
                            <option value="custom_range" {{ old('validity_mode', 'custom_range') === 'custom_range' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'فترة مخصصة (يدوي)' : 'Custom Range' }}</option>
                            <option value="current_year" {{ old('validity_mode') === 'current_year' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'السنة الحالية تلقائيًا' : 'Current Year Automatically' }}</option>
                            <option value="specific_year" {{ old('validity_mode') === 'specific_year' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'سنة محددة' : 'Specific Year' }}</option>
                        </select>
                        @error('validity_mode')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-row" id="year_row" style="display:none;">
                        <div class="form-group col-md-4">
                            ...
                            <input type="number" class="form-control @error('valid_year') is-invalid @enderror" id="valid_year" name="valid_year"
                                   min="2000" max="2100" value="{{ old('valid_year', now()->year) }}">
                            @error('valid_year')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="form-row" id="date_range_row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="date" class="form-control @error('active_from') is-invalid @enderror"
                                   id="active_from" name="active_from" value="{{ old('active_from') }}">
                            @error('active_from')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="date" class="form-control @error('active_to') is-invalid @enderror"
                                   id="active_to" name="active_to" value="{{ old('active_to') }}">
                            @error('active_to')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="page-form-section">
                    ...
                    <div class="form-group">
                        <label for="content">{{ __('admin-dashboard.page_content') }}</label>
                        <div class="editor-shell">
                            <div id="content_editor" class="quill-editor" data-initial='@json(old("content", ""))'></div>
                        </div>
                    ...
                    <div class="form-group mb-0">
                        <label for="content_ar">{{ app()->getLocale() === 'ar' ? 'محتوى الصفحة (عربي)' : 'Page Content (Arabic)' }}</label>
                        <div class="editor-shell">
                            <div id="content_ar_editor" class="quill-editor" data-initial='@json(old("content_ar", ""))'></div>
                        </div>
                        <textarea class="d-none @error('content_ar') is-invalid @enderror" id="content_ar" name="content_ar">{{ old('content_ar') }}</textarea>
                        @error('content_ar')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="d-flex justify-content-end mt-3">
                    <button type="submit" class="btn btn-primary px-4">{{ __('admin-dashboard.create_page') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    (function () {
        function initQuillEditors() {
            ...
            function initQuill(editorId, inputId) {
                ...
                const initialHtml = editorElement.dataset.initial || '';
