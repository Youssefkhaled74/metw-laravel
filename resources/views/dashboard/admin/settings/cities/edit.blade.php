...
@section('title', app()->getLocale() === 'ar' ? 'تعديل مدينة' : 'Edit City')
                ...
                <div class="card-body p-4">
                    <form action="{{ route('admin.settings.cities.update', $city->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name_en" value="{{ old('name_en', $city->name_en) }}" class="form-control @error('name_en') is-invalid @enderror" required>
                            @error('name_en')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label">{{ app()->getLocale() === 'ar' ? 'اسم المدينة بالعربية' : 'City name in Arabic' }}</label>
                            <input type="text" name="name_ar" value="{{ old('name_ar', $city->name_ar) }}" class="form-control @error('name_ar') is-invalid @enderror" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="governorate_id" class="form-select @error('governorate_id') is-invalid @enderror" required>
                                ...
                                @foreach ($governorates as $governorate)
                                    <option value="{{ $governorate->id }}" @selected(old('governorate_id', $city->governorate_id) == $governorate->id)>{{ $governorate->name_ar }}</option>
                                @endforeach
                            </select>
