...
@section('title', __('admin-dashboard.delivery_types_management'))
@section('page-title', __('admin-dashboard.delivery_types_management'))

@section('page-actions')
    <a href="{{ route('admin.settings.delivery-types.create') }}" class="btn btn-primary">
        <i class="fas fa-plus"></i> {{ __('admin-dashboard.add_new_delivery_type') }}
    </a>
@endsection
    ...
    <div class="card shadow-sm border-0 data-card">
        <div class="card-header bg-white border-0 py-3">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                <h5 class="mb-0">{{ __('admin-dashboard.delivery_types_management') }}</h5>
                <span class="badge rounded-pill text-bg-light border text-muted px-3 py-2 rows-counter-badge">
                    <i class="fas fa-shipping-fast me-2"></i>
                    {{ $deliveryTypes->count() }} / {{ $deliveryTypes->total() }}
                </span>
            </div>

            <form method="GET" action="{{ route('admin.settings.delivery-types.index') }}" class="row g-2 align-items-center">
                <div class="col-lg-8">
                ...
                <div class="col-lg-4 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.settings.delivery-types.index') }}" class="btn btn-outline-secondary btn-sm">
        ...
        <div class="card-body p-0 table-wrap">
            @if ($deliveryTypes->count() > 0)
                        ...
                        <thead>
                            <tr>
                                <th class="text-nowrap sortable-col">
                                    ...
                                    @endphp
                                    <a class="text-decoration-none text-reset" href="{{ route('admin.settings.delivery-types.index', array_merge(request()->except('page'), ['sort_by' => 'id', 'sort_dir' => $idDir])) }}">
                                ...
                                <th class="text-nowrap sortable-col">
                                    ...
                                    @endphp
                                    <a class="text-decoration-none text-reset" href="{{ route('admin.settings.delivery-types.index', array_merge(request()->except('page'), ['sort_by' => 'name', 'sort_dir' => $nameDir])) }}">
                        ...
                        <tbody>
                            @foreach ($deliveryTypes as $type)
                                <tr>
                                    ...
                                    <td>
                                        <div class="fw-semibold text-dark entity-name">{{ $type->name }}</div>
                                        @if($type->description)
                                            <small class="text-muted d-block entity-subname">{{ \Illuminate\Support\Str::limit($type->description, 80) }}</small>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="actions-group">
                                            <form action="{{ route('admin.settings.delivery-types.toggle-status', $type) }}" method="POST" class="d-inline m-0"
                                            ...
                                            <a href="{{ route('admin.settings.delivery-types.edit', $type) }}" class="btn btn-sm btn-primary text-white action-icon-btn"
                                               title="{{ __('admin-dashboard.edit') }}" data-bs-toggle="tooltip" data-bs-placement="top">
                                                <i class="fas fa-edit"></i>
                                            </a>

                                            <form action="{{ route('admin.settings.delivery-types.destroy', $type) }}" method="POST" class="d-inline m-0"
                                                  onsubmit="return confirm('{{ __('admin-dashboard.confirm_delete_delivery_type') }}');">
                                                @csrf
                                                @method('DELETE')
                ...
                <div class="d-flex justify-content-center mt-4 mb-3">
                    {{ $deliveryTypes->links('pagination::bootstrap-5') }}
                </div>
            @else
                <div class="text-center py-5">
                    <div class="empty-state d-inline-block px-4 py-5">
                        <i class="fas fa-shipping-fast empty-icon mb-3"></i>
                        <h5 class="text-muted">{{ __('admin-dashboard.no_delivery_types_found') }}</h5>
                        <p class="text-muted mb-0">{{ app()->getLocale() === 'ar' ? 'لا يوجد أنواع توصيل مضافة حالياً.' : 'No delivery types are available at the moment.' }}</p>
                        <div class="mt-3">
                            <a href="{{ route('admin.settings.delivery-types.create') }}" class="btn btn-primary">
                                <i class="fas fa-plus"></i> {{ __('admin-dashboard.add_new_delivery_type') }}
                            </a>
