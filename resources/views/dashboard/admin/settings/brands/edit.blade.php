...
@section('title', __('admin-dashboard.edit_brand'))
@section('page-title', __('admin-dashboard.edit_brand'))
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">{{ __('admin-dashboard.edit_brand') }}</li>
@endsection
...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">{{ __('admin-dashboard.edit_brand_details') }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.settings.brands.update', $brand->id) }}"
                          method="POST"
                        ...
                        <div class="mb-3">
                            <label for="image" class="form-label">{{ __('admin-dashboard.brand_image') }}</label>
                            <input type="file"
                                   class="form-control @error('image') is-invalid @enderror"
                                   id="image"
                                   name="image"
                                   accept="image/*">
                            <div class="form-text">{{ __('admin-dashboard.keep_current_image') }} {{ __('admin-dashboard.brand_logo_size') }}</div>
                            @error('image')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> {{ __('admin-dashboard.update_brand') }}
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <div class="col-md-4">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">{{ __('admin-dashboard.current_logo') }}</h5>
                </div>
                <div class="card-body">
                    <div class="text-center" id="imagePreview">
                        <img src="{{ asset($brand->image) }}"
                             alt="{{ __('admin-dashboard.current_logo') }}"
