...
@section('title', __('admin-dashboard.edit_zone'))
@section('page-title', __('admin-dashboard.edit_zone'))
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">{{ __('admin-dashboard.edit_zone') }}</li>
@endsection
...
<div class="row justify-content-center">
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.settings.zones.update', $zone->id) }}" method="POST">
                    @csrf
                    @method('PATCH')

                    <div class="mb-3">
                        <label for="name_en" class="form-label required">{{ __('admin-dashboard.zone_name_en') }} <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('name_en') is-invalid @enderror" id="name_en" name="name_en" value="{{ old('name_en', $zone->name_en) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('name_ar') is-invalid @enderror" id="name_ar" name="name_ar" value="{{ old('name_ar', $zone->name_ar) }}" required>
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch">
                            ...
                            <label class="form-check-label" for="is_active">{{ __('admin-dashboard.active') }}</label>
                                        ...
                                        <div class="text-end">
                        <a href="{{ route('admin.settings.zones.index') }}" class="btn btn-light me-2">{{ __('admin-dashboard.cancel') }}</a>
                        <button type="submit" class="btn btn-primary">{{ __('admin-dashboard.update_zone') }}</button>
                    </div>
                </form>
            </div>
        </div>
